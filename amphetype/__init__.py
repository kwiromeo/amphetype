...
AMPH_DIR = Path(LIB_DIR).parent
DATA_DIR = AMPH_DIR / "data"
VERSION_FILE = LIB_DIR / "VERSION"

if not VERSION_FILE.is_file():
  raise RuntimeError(f"file {VERSION_FILE} not found")
...
""",
  ...
  p.add_argument("-d", "--database", metavar="DBFILE", help="uses the database file %(metavar)s")
  p.add_argument("-s", "--settings", metavar="INIFILE", help="uses settings file %(metavar)s")
  p.add_argument(
    "-L", "--log", metavar="LOGFILE", help="""enables logging to the given file; use "-" to log to stdout"""
  )
  ...
  logging.basicConfig(format="%(asctime)s :: %(levelname)s :: %(message)s", **logargs)
